...
pub struct ModeSense6Command {
    #[packed(start_bit=7, end_bit=0, start_byte=0, end_byte=0)] pub op_code: u8,
...
mod tests {
    ...
    fn test_bits<S: Bit, E: Bit>(width: usize) {
        ...
        match width {
            1 => {
                assert_eq!(expected as u16, <u16 as Packed<&[u8], S, E, U1>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u32, <u32 as Packed<&[u8], S, E, U1>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U1>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U1>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            2 => {
                assert_eq!(expected as u16, <u16 as Packed<&[u8], S, E, U2>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u32, <u32 as Packed<&[u8], S, E, U2>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U2>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U2>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            3 => {
                assert_eq!(expected as u32, <u32 as Packed<&[u8], S, E, U3>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U3>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U3>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            4 => {
                assert_eq!(expected as u32, <u32 as Packed<&[u8], S, E, U4>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U4>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U4>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            5 => {
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U5>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U5>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            6 => {
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U6>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U6>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            7 => {
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U7>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U7>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            8 => {
                assert_eq!(expected as u64, <u64 as Packed<&[u8], S, E, U8>>::unpack::<LittleEndian>(bytes).unwrap());
                assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U8>>::unpack::<LittleEndian>(bytes).unwrap());
            },
            15 => assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U15>>::unpack::<LittleEndian>(bytes).unwrap()),
            16 => assert_eq!(expected as u128, <u128 as Packed<&[u8], S, E, U16>>::unpack::<LittleEndian>(bytes).unwrap()),
            _ => {},
        }
